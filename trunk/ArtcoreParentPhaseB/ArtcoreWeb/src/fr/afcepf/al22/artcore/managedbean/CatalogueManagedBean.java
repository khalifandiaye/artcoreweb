...
public class CatalogueManagedBean {
    ...
    /**
     * Int qui fait office de booléen ; il va permettre de choisir quelle liste
     * mettre dans le getter. S'il est égal à 0, on affiche la liste complète
     * des produits à la vente ; S'il est égal à 1, on affiche la liste par nom;
     * S'il est égal à 2, on affiche la liste par catégorie; S'il est égal à 3,
     * on affiche la liste par format; S'il est égal à 4, on affiche la liste
     * par prix; S'il est égal à 5, on affiche la liste part thème; S'il est
     * égal à 6, on affiche la liste par nom d'artiste S'il est égal à 7, on
     * affiche la liste multicritères (codée dans le mb); S'il est égal à 8, on
     * affiche vraiment tous les produits
     */
    ...
    /**
     * Méthode qui modifie réellement le produit. Redirige vers le catalogue.
     * 
     * @return
     */
    ...
    /**
     * Méthode pour retirer le produit de la vente. Redirige vers le calalogue à
	...
	log.info("Suppression produit " + i);
	return "catalogueAdmin.xhtml";
    }

    /**
     * Méthode qui récupère un produit sélectionné. Sert à l'affichage du détail
     * d'un produit. A VOIR CE QU'ON CODE DEDANS.
     * 
     * @return
     */
	...
	case 1:
	    listeProduits = buRechercheProduit
		    .rechercherProduitsParNom(labelRechercheNom);
	    return listeProduits;
	case 2:
		...
		listeProduits = buRechercheProduit
			.produitsParCategorie(intRechercheCategorie);
	    }
	    return listeProduits;
	case 3:
		...
		listeProduits = buRechercheProduit
			.produitsParFormat(intRechercheFormat);
	    }
	    return listeProduits;
	case 4:
	    listeProduits = buRechercheProduit.produitsParPrix(prixMin,
		    prixInter, prixMax);
	    return listeProduits;
	case 5:
	case 6:
	    listeProduits = buRechercheProduit
		    .produitsParArtiste(labelRechercheNomArtiste);
	    return listeProduits;
	case 7:
	    listeProduits = buRechercheProduit.produitsMultiCriteres(
		    labelRechercheNom, labelRechercheNomArtiste,
		    intRechercheCategorie, intRechercheFormat, prixMin,
		    prixInter, prixMax);
	    return listeProduits;
	case 8:
	...
	log.debug("la liste de categorie contient " + listeCategories.size()
		+ " items.");
