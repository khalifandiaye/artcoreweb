...
public class BusinessPanierImpl implements IBusinessPanier {
    ...
    /*
     * ListBlocProduit represente le panier . un BlocProduitDto est un Objet qui
     * a un attribut produit,quantite et prixTotal
     */
	...
	for (BlocProduitDto blocProduitDto : ListBlocProduit) {
	    if (blocProduitDto.getProduit().getIdProduit() == produit
		    .getIdProduit()) {
		inexistant = false;
	...
	for (BlocProduitDto blocProduitDto : ListBlocProduit) {
	    if (blocProduitDto.getProduit().getIdProduit() == produiDtoProduit
		    .getIdProduit()) {
    ...
    /*
     * calcul du prix total : prix total eqgale a la somme du prix total des
     * lignes de commande
     */
    @Override
	...
	if (ListBlocProduit != null && !ListBlocProduit.isEmpty()) {
	    ...
	    log.debug("Business : Le panier a une taille de : "
		    + ListBlocProduit.size());
	    for (BlocProduitDto blocProduitDto : ListBlocProduit) {
	 ...
	 */
	    ...
	    log.debug("Business : maintenant il est censé être à 0 : "
		    + prixTotal);
	}
	return prixTotal;
    }
